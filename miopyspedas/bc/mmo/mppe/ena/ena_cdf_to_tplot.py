    ...
    for filename in filenames:
        ...
        for var in load_cdf_variables:
            ...
            if this_var_type in var_type:
                ...
                if "DEPEND_4" in var_atts:
                    if y_ndims < 5:
                        # TWINS imager data has this
                        logging.warning("Variable %s has only %d dimensions (including time), but has a DEPEND_4 attribute. Removing attribute.", var, y_ndims)
                        depend_4 = None
                    elif var_atts["DEPEND_4"] in master_cdf_variables:
                        try:
                            # Check for correct shape, matching time and data dimensions
                            dep_name = var_atts["DEPEND_4"]
                            depend_4 = np.array(master_cdf_file.varget(dep_name))
                            # String-valued DEPEND_N handling
                            # This is not strictly ISTP compliant, but it's extremeley common. For
                            # example, vector-valued data will often have DEPEND_1 values that are more like
                            # labels, i.e. ['x', 'y', 'z']
                            ...
                            # The situation is further complicated by the fact that cdflib seems to introduce
                            # extra leading or trailing dimensions on string-valued variables.
                            if depend_4 is not None and depend_4.dtype.type is np.str_:
                                # Get the original array dimensions from the variable properties
                                dp_props = master_cdf_file.varinq(dep_name)
                                if new_cdflib:
                                    orig_dimensions = dp_props.Dim_Sizes
                                    ...
                                    orig_dimensions = dp_props['Dim_Sizes']
                                reshape_dim = tuple(orig_dimensions)
                                depend_4 = np.reshape(depend_4, reshape_dim)
                                #depend_4 = None
                                pass

                            dep_dims = depend_4.shape
                            dep_ndims = len(dep_dims)
                            if dep_ndims == 0:
                                logging.warning("Variable %s DEPEND_4 attribute %s is zero-dimensional. Removing attribute.", var,
                                                dep_name)
                                depend_4 = None
                            elif dep_ndims == 1:
                                # Not time varying
                                ...
                                # Too many dimensions
                                logging.warning(
                                    "Variable %s DEPEND_4 attribute %s has too many dimensions (%d). Removing attribute.",
                                    var, dep_name, dep_ndims)
                                depend_4 = None
                        except ValueError:
                            logging.warning('Unable to get DEPEND_4 variable %s while processing %s',
                                            var_atts["DEPEND_4"], var)
                ...
                if ndims >= 4 and depend_3 is None:
                    ...
                    pass
                if ndims >= 5 and depend_4 is None:
                    #logging.warning("Variable %s has %d dimensions, but no DEPEND_4, adding index range for dimension 4", var_name, ndims )
                    #depend_4 = np.arange(ydata.shape[4])
                    pass

                if depend_1 is not None and depend_2 is not None and depend_3 is not None and depend_4 is not None:
                    tplot_data['v1'] = depend_1
                    tplot_data['v2'] = depend_2
                    tplot_data['v3'] = depend_3
                    tplot_data['v4'] = depend_4
                    ...
                    if len(depend_4.shape) == 1:
                        nontime_varying_depends.append('v4')

                elif depend_1 is not None and depend_2 is not None and depend_3 is not None:
                    tplot_data['v1'] = depend_1
                    tplot_data['v2'] = depend_2
                    tplot_data['v3'] = depend_3
                ...
                if 'DEPEND_4' in var_atts:
                    if isinstance(var_atts['DEPEND_4'], str):
                        try:
                            depend_4_var_atts = master_cdf_file.varattsget(var_atts['DEPEND_4'])
                            depend_4_units = depend_4_var_atts.get('UNITS')
                            if depend_4_units is not None:
                                metadata[var_name]['DEPEND_4_UNITS'] = depend_4_units
                        except ValueError:
                            # some variables aren't actually available
                            pass
        ...
        try:
            ...
            if metadata.get(var_name) is not None:
                ...
                attr_dict["data_att"] = {"coord_sys": "",
                                         ...
                                         "depend_4_units": metadata[var_name].get('DEPEND_4_UNITS')}
